...
const Likewatchlatersavbtns = ({ vv, vid }) => {
    ...
    useEffect(() => {
        ...
        if (likedvideolist?.data?.some((q) => q.videoid === vid && q.viewer === currentUser.result._id)) {
            setlikebtn(true);
        }

        if (watchlaterlist?.data?.some((q) => q.videoid === vid && q.viewer === currentUser.result._id)) {
            setsavevideo(true);
        }
...
export default Likewatchlatersavbtns;